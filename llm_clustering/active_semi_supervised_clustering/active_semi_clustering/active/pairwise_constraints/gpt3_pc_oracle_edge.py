...
def construct_pairwise_oracle_single_example(document_i, document_j, label, dataset_name, prompt_suffix = None, text_type = None, add_label=True):
    if prompt_suffix is None:
        if dataset_name == "OPIEC59k":
            ...
            prompt_suffix = "express the same emotion?"
            ...
            # prompt_suffix = "express the same general intent?"
            # prompt_suffix = "correspond to the same general domain?"
            prompt_suffix = "belong to the same type?"
...
class GPT3Oracle_edge:
    def __init__(self, X, prompt, documents, dataset_name=None, prompt_suffix=None, text_type=None, cache_file=None, max_queries_cnt=2500, num_predictions=5, read_only=False):
        ...
        for row in self.cache_rows:
            # print(row)
            sorted_pair_list = sorted([row["context1"], row["context2"]])
            self.gpt3_pairwise_labels[tuple(sorted_pair_list)] = row["labels"]

        self.prompt = prompt

    @staticmethod
    def filter_high_entropy_predictions(pair_labels):
        """
        Return the value (True or False) that appears most frequently in the input list, ignoring None values.

        Parameters:
        - pair_labels: A list containing boolean values (True/False) and possibly None.

        Returns:
        - True if True appears more frequently than False.
        - False if False appears more frequently than True.
        - None if the counts are equal or the list contains only None values.
        """
        # Filter out None values
        filtered_labels = [label for label in pair_labels if label is not None]

        # Count True and False values
        true_count = sum(filtered_labels)  # Count True values
        false_count = len(filtered_labels) - true_count  # Count False values
