...
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.exceptions import EmptyClustersException
from .constraints import preprocess_constraints, preprocess_constraints_no_transitive_closure, preprocess_constraints_with_weights
...
class WeightedPCKMeans(KMeans):
    ...
    def fit(self, X, y=None, ml=[], cl=[], weight_method="cosine"):
        ...
        for iteration in range(self.max_iter):
            ...
            labels = self._assign_clusters(X, cluster_centers, ml_graph, cl_graph, ml_weights, cl_weights)
    ...
    def _objective_function(self, x_i, point_cluster_distances, c_i, labels, ml_graph, cl_graph, ml_weights, cl_weights, print_terms=False):
        distance = 1 / 2 * point_cluster_distances[x_i, c_i]

        ml_penalty = 1
        for y_i in ml_graph[x_i]:
            if labels[y_i] != -1 and labels[y_i] != c_i:
                ml_penalty += ml_weights[(x_i, y_i)]

        cl_penalty = 1
        for y_i in cl_graph[x_i]:
            if labels[y_i] == c_i:
                cl_penalty += cl_weights[(x_i, y_i)]

        if print_terms:
            metric_dict = {"x_i": x_i, "distance": round(distance, 4), "ml_penalty": round(ml_penalty, 4), "cl_penalty": round(cl_penalty, 4)}
    ...
    def _assign_clusters(self, X, cluster_centers, ml_graph, cl_graph, ml_weights, cl_weights):
        ...
        for x_i in index:
            cluster_distances = [
                self._objective_function(x_i, point_cluster_distances, c_i, labels, ml_graph, cl_graph, ml_weights, cl_weights)[0]
