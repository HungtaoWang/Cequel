...
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.farthest_first_traversal import weighted_farthest_first_traversal
...
class MPCKMeansMF:
    ...
    def _objective_function(self, X, i, labels, cluster_centers, cluster_id, As, farthest, ml_graph, cl_graph, w):
        ...
        # term_d = self._dist(X[i], cluster_centers[cluster_id], A) - logdet / np.log(2)
        # sign, logdet = np.linalg.slogdet(As[cluster_id])
        # term_d = self._dist(X[i], cluster_centers[cluster_id], As[cluster_id]) - logdet / np.log(2)

        sign, logdet = np.linalg.slogdet(As[cluster_id])
        # 用最大值确保行列式不会为零，类似第一段代码中的处理
        logdet = np.log(max(np.exp(logdet), 1e-9))  # np.exp(logdet) 还原回行列式值

        term_d = self._dist(X[i], cluster_centers[cluster_id], As[cluster_id]) - logdet
