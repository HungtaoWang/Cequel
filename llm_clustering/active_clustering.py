...
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.semi_supervised.pairwise_constraints import PCKMeans, CardinalityConstrainedPCKMeans, GPTExpansionClustering, KMeansCorrection
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.semi_supervised.labeled_data.kmeans import KMeans
# from sklearn.cluster import KMeans
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.semi_supervised.labeled_data.seededkmeans import SeededKMeans
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.semi_supervised.labeled_data.constrainedkmeans import ConstrainedKMeans
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.active.pairwise_constraints import ExampleOracle, GPT3Oracle, construct_pairwise_oracle_single_example, GPT3ComparativeOracle, DistanceBasedSelector, LabelBasedSelector, ExploreConsolidate, MinMax, SimilarityFinder, MinMaxFinetune
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.active.pairwise_constraints import Random

from llm_clustering.cmvc.helper import invertDic
from llm_clustering.cmvc.metrics import pairwiseMetric, calcF1
from llm_clustering.cmvc.test_performance import cluster_test
from llm_clustering.cmvc.model_max_margin import KGEModel
from llm_clustering.cmvc.Context_view import BertClassificationModel

from llm_clustering.eval_utils import cluster_acc
...
def cluster(semisupervised_algo, features, documents, labels, num_clusters, dataset_name, text_type=None, prompt_suffix=None, num_corrections=None, split=None, init="random", max_feedback_given=None, normalize_vectors=False, split_normalization=False, num_reinit=1, verbose=False, side_information=None, process_raw_data=False, pckmeans_w=None, seed=None):
    pairwise_constraint_cache_name = f"/projects/ogma2/users//extra_storage/okb-canonicalization/clustering/file/gpt3_cache/{dataset_name}_pairwise_constraint_cache.jsonl"
    sentence_unprocessing_mapping_file = f"/projects/ogma2/users//extra_storage/okb-canonicalization/clustering/file/gpt3_cache/{dataset_name}_{split}_sentence_unprocessing_map.json"
    ...
    if semisupervised_algo == "KMeans":
        ...
        labels_cache_file = f"/projects/ogma2/users//extra_storage/okb-canonicalization/clustering/output/{dataset_name}_kmeans_labels.json"
        cluster_centers_cache_file = f"/projects/ogma2/users//extra_storage/okb-canonicalization/clustering/output/{dataset_name}_kmeans_cluster_centers.npy"
        ...
        oracle = GPT3Oracle(features, prompt, documents, dataset_name=dataset_name, prompt_suffix=prompt_suffix, text_type=text_type, max_queries_cnt=max_feedback_given, cache_file = f"/projects/ogma2/users//extra_storage/okb-canonicalization/clustering/file/gpt3_cache/{dataset_name}_pairwise_constraint_cache.jsonl")
        ...
        cache_file_name = f"/projects/ogma2/users//extra_storage/okb-canonicalization/clustering/file/gpt3_cache/{dataset_name}_gpt_paraphrase_cache.jsonl"
        ...
        prompt = construct_pairwise_oracle_prompt(dataset_name, documents, side_information)
        oracle = GPT3Oracle(features, prompt, documents, dataset_name=dataset_name, prompt_suffix=prompt_suffix, text_type=text_type, max_queries_cnt=max_feedback_given, cache_file = f"/projects/ogma1//few-shot-clustering/clustering/file/gpt3_cache/{dataset_name}_pairwise_constraint_cache_reprod.jsonl")
        ...
        oracle = ExampleOracle(labels, max_queries_cnt=max_feedback_given)
        prompt = construct_pairwise_oracle_prompt(dataset_name, documents, side_information)
        gpt3_oracle = GPT3Oracle(features, prompt, documents, dataset_name=dataset_name, prompt_suffix=prompt_suffix, text_type=text_type, max_queries_cnt=max_feedback_given, cache_file = f"/projects/ogma2/users//extra_storage/okb-canonicalization/clustering/file/gpt3_cache/{dataset_name}_pairwise_constraint_cache.jsonl")
...
def compare_algorithms(features,
    ...
    for i, seed in enumerate(range(num_seeds)):
        ...
        for semisupervised_algo in algorithms:
            ...
            # np.save(open("/projects/ogma1//okb-canonicalization/clustering/output/OPIEC59k_test_1/OPIEC59k_clusters/kmeans/cluster_centers.npy", 'wb'), clusterer.cluster_centers_)
            # 
            # breakpoint()
            metric_dict = {}
