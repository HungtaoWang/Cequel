...
from llm_clustering.dataloaders import load_dataset, generate_synthetic_data
from llm_clustering.experiment_utils import set_seed, summarize_results

from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.semi_supervised.pairwise_constraints import PCKMeans, GPTExpansionClustering
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.active.pairwise_constraints import GPT3Oracle_edge, GPT3Oracle_triangle, DistanceBasedSelector, SimilarityFinder, TriangleFinder
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.semi_supervised.pairwise_constraints import COPKMeans, CardinalityConstrainedPCKMeans
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.semi_supervised.pairwise_constraints import MPCKMeans, MPCKMeansMF
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.semi_supervised.pairwise_constraints import MKMeans, RCAKMeans
from llm_clustering.active_semi_supervised_clustering.active_semi_clustering.semi_supervised.pairwise_constraints import WeightedPCKMeans

from collections import Counter

def LLMPairwiseClustering(features, documents, num_clusters, prompt, text_type, prompt_suffix, max_feedback_given=10000, pckmeans_w=0.4, cache_file=None, constraint_selection_algorithm="SimilarityFinder", kmeans_init="k-means++", mode="largest", data="bbc", selection="edge", clustering="PCKMeans", weight_method="cosine", encoder_name="instructor"):
    ...
    """
    if selection == "edge":
        oracle = GPT3Oracle_edge(features,
                            prompt,
                            documents,
                            dataset_name=None,
        ...
        oracle = GPT3Oracle_triangle(features,
                            prompt,
                            documents,
                            dataset_name=None,
                            prompt_suffix=prompt_suffix,
    ...
    if constraint_selection_algorithm == "SimilarityFinder":
        if selection == "edge":
            active_learner = SimilarityFinder(n_clusters=num_clusters, mode=mode, data=data, selection=selection, encoder_name=encoder_name)
        elif selection == "triangle":
            active_learner = TriangleFinder(n_clusters=num_clusters, mode=mode, data=data, selection=selection, encoder_name=encoder_name)
    else:
    ...
    if clustering == "WeightedPCKMeans":
        print("Training WeightedPCKMeans")
        clusterer = WeightedPCKMeans(n_clusters=num_clusters)
        clusterer.fit(features, ml=pairwise_constraints[0], cl=pairwise_constraints[1], weight_method=weight_method)
    elif clustering == "WeightedConstrainedSpectralClustering":
        print("Returning constraints")
        return None, pairwise_constraints
    # clusterer.fit(features, ml=pairwise_constraints[0], cl=pairwise_constraints[1])
    clusterer.constraints_ = pairwise_constraints
    if selection == "edge":
        if isinstance(oracle, GPT3Oracle_edge) and os.path.exists(oracle.cache_file):
            oracle.cache_writer.close()
    elif selection == "triangle":
        if isinstance(oracle, GPT3Oracle_triangle) and os.path.exists(oracle.cache_file):
            oracle.cache_writer.close()
